...
class TimetableWidgetNormal : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (context != null && intent != null) {
            ...
            if (action[0] == "updateNormalWidget") {
    ...
    private fun updateWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
        val views = RemoteViews(context.packageName, R.layout.timetable_widget_normal)
        views.setOnClickPendingIntent(R.id.widgetUpdate, pendingIntent(context, "updateNormalWidget|$appWidgetId"))
        try {
            ...
            views.setTextViewText(R.id.widgetNowTitle, if (nowTime > 0) { context.getString(R.string.widget_now) } else { context.getString(R.string.widget_earlier) } )
            views.setTextViewText(R.id.widgetNowTimeTitle, if (nowTime > 0) { context.getString(R.string.widget_now_time) } else { context.getString(R.string.widget_earlier_time) } )

            val colorNow  =
                if (nowTime > 0) { ContextCompat.getColor(context, R.color.gray_light) }
                else { ContextCompat.getColor(context, R.color.gray) }
            val colorThen =
                if (nowTime > 0 || timetable.thenDays > 0) { ContextCompat.getColor(context, R.color.gray) }
