...
class WidgetLesson : AppWidgetProvider() {
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        for (appWidgetId in appWidgetIds) updateLessonWidget(context, appWidgetManager, appWidgetId)
    }
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (context != null && intent != null) {
            ...
            if (action[0] == "update") updateLessonWidget(context, AppWidgetManager.getInstance(context), action[1].toInt())
        }
    }

    private fun updateLessonWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
        val views = RemoteViews(context.packageName, R.layout.widget_small)
        val intent = Intent(context, WidgetLesson::class.java)
